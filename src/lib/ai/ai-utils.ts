...
export async function askBedrock(question: string): Promise<string> {
  const modelId = "qwen.qwen3-coder-30b-a3b-v1:0";

  const system_prompt =
    "You are a helpful assistant that specializes in League of Legends data analysis and game insights. Provide clear, concise answers about match data, champion statistics, and gameplay analysis.";
  ...
  return assistantContent ?? "No response found";
}
